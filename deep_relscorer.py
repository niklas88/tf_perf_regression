...
class DeepCNNRelScorer():
    ...
    def init_from_config(use_type_names=True, use_attention=True,
        ...
        logdir = config_options.get('DeepRelScorer',
                                    'logdir')
        return DeepCNNRelScorer(logdir, embeddings_file,
    ...
    def extract_vectors(self, gensim_model_fname):
        ...
        PAD_ID = 0
        vocab[DeepCNNRelScorer.PAD] = PAD_ID

        # Vector for UNK
        UNK_ID = 1
        vocab[DeepCNNRelScorer.UNK] = UNK_ID
    ...
    def learn_relation_model(self, train_pos, train_neg, extend_model=None,
        ...
        if extend_model:
            ...
            self.UNK_ID = self.vocab[DeepCNNRelScorer.UNK]
            self.init_new_model()
    ...
    def build_deep_model(self):
        ...
        with tf.device('/cpu:0'), tf.name_scope("load_embeddings"):
            ...
            #    name="Wembed",
            #    trainable=False)
            # need to align the embedding for TF 1.6
            self.embeddings = align_numpy_tf(self.embeddings)
